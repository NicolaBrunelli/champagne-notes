...
export default async (request: Request) => {
  ...
  if (!['POST', 'PATCH', 'PUT', 'DELETE'].includes(request.method)) return json({ error: 'Metodo non supportato.' }, 405);
  ...
  try {
    ...
    if (request.method === 'PUT') {
      ...
      if (existing.redeemedAt) return json({ error: 'Un invito già utilizzato non può essere rigenerato.' }, 409);
      const code = createCode();
      const updated: InviteRecord = { ...existing, code, codeHash: await hashCode(code) };
      await saveInvite(updated);
      return json({ invite: publicInvite(updated), code });
    }
...
export const config = { path: '/api/admin/inviti', method: ['GET', 'POST', 'PATCH', 'PUT', 'DELETE'] };
